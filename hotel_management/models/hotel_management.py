...
class hotel_reservation(models.Model):
	...
	def onchange_pricelist_id(self):
		...
		if 'checkin' in self._context and 'checkout' in self._context:
			...
			if policy_obj.time :
				checkout_time = str(policy_obj.time)+':00:00'
				checkout_format = '%Y-%m-%d '+checkout_time
				checkin_time = str(int(policy_obj.time)+2)+':00:00'
				checkin_format = '%Y-%m-%d '+checkin_time

			else:
				checkout_time = '13:00:00'
				checkout_format = '%Y-%m-%d '+checkout_time
				checkin_time = '15:00:00'
				checkin_format = '%Y-%m-%d '+checkin_time

			chin_dt = self._context['checkin'].replace('00:00:00',checkin_time)
			chout_dt = self._context['checkout'].replace('00:00:00',checkout_time)
			user_tz = self._context.get('tz') or self.env.user.tz or 'UTC'
			tz_session = pytz.timezone(user_tz)

			checkin_obj = datetime.strptime(chin_dt, '%Y-%m-%d %H:%M:%S').astimezone(tz_session)
			checkin_dt = checkin_obj.strftime('%Y-%m-%d %H:%M:%S')

			checkout_obj = datetime.strptime(chout_dt, '%Y-%m-%d %H:%M:%S').astimezone(tz_session)
			checkout_dt = checkout_obj.strftime('%Y-%m-%d %H:%M:%S')

			ctx.update({'date': checkin_dt})
			res_line = {
	...
	def send_mail(self):
		template = self.env.ref(
			'hotel_management.hotel_reservation_confirmed_mail_template')
		# values = {
		...
		# sending = template.send_mail(self.id,
		# 							 raise_exception=True, email_values=values)

		template_id = self.env['ir.model.data']._xmlid_lookup('hotel_management.hotel_reservation_confirmed_mail_template')[2]
		email_template_obj = self.env['mail.template'].browse(template_id)
		
		if template_id:
			values = email_template_obj.generate_email(self.id, fields=['subject', 'body_html', 'email_from', 'email_to'])
			values['email_from'] = self.partner_id.email_formatted
			values['email_to'] = self.company_id.partner_id.email_formatted
...
class hotel_reservation_line(models.Model):
	...
	shop_id = fields.Many2one('sale.shop', 'Shop',related="line_id.shop_id")
	...
	def default_get(self, default_fields):
		...
		policy_obj = self.env['checkout.configuration'].sudo().search(
			[('shop_id', '=', reservation_id.shop_id.id)])
		...
		if policy_obj.time :
			...
			checkout_format = '%Y-%m-%d 13:00:00'
			checkin_format = '%Y-%m-%d 15:00:00'

		user_tz = self._context.get('tz') or self.env.user.tz or 'UTC'
		tz_session = pytz.timezone(user_tz)
		today_dt = date.today()

		dt_in = today_dt.strftime(checkin_format)
		checkin_obj = datetime.strptime(dt_in, '%Y-%m-%d %H:%M:%S')
		checkin_dt = checkin_obj.astimezone(tz_session)
		in_time = pytz.utc.localize(checkin_obj).astimezone(tz_session)


		dt = (today_dt + timedelta(days=1)).strftime(checkout_format)
		checkout_obj = datetime.strptime(dt, '%Y-%m-%d %H:%M:%S')
		checkout_dt = checkout_obj.astimezone(tz_session)
		out_time = pytz.utc.localize(checkout_obj).astimezone(tz_session)

		print(in_time,"in------------------------",out_time)
		print(checkin_dt,"3333333333------------------------",checkout_dt)
		...
		# else:
		res['checkin'] = checkin_dt.strftime('%Y-%m-%d %H:%M:%S')
		res['checkout'] = checkout_dt.strftime('%Y-%m-%d %H:%M:%S')
...
class hotel_folio(models.Model):
	...
	@api.depends('room_lines','room_lines.product_id')
	def rooms_reference(self):
		for order in self:
			order.rooms_ref = order.rooms_ref1 = order_ref = ''
			for line in order.room_lines:
				if line.product_id:
					order_ref += line.product_id.name + ','
			print(order,"otder_ref-------------",order_ref)
			if order_ref and order_ref[-1] == ',':
				order.rooms_ref = order_ref[:-1]
				order.rooms_ref1 = order_ref[:-1]
			else:
				order.rooms_ref = order_ref
				order.rooms_ref1 = order_ref
	...
	rooms_ref = fields.Char(compute="rooms_reference", string="Room No", store=True)
	rooms_ref1 = fields.Char(compute="rooms_reference",
	...
	def action_done(self):
		for line in self:
			for invoice in line.invoice_ids:
				if (invoice.partner_id.id == line.partner_id.id) and invoice.payment_state not in ['in_payment','paid'] :
