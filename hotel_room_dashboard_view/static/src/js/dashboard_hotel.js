odoo.define('hotel_room_dashboard_view.dashboard_hotel', function (require) {
	...
	var HotelDashboardView = AbstractAction.extend({
			...
			$.when(this.load_hotel()).then(function () {
				...
				for (var res_line_id = 0; res_line_id < self.reservation_lines1.length; res_line_id++) {
					...
					for (var res_line = 0; res_line < self.reservations1.length; res_line++) {
						if (self.reservations1[res_line].id == self.reservation_lines1[res_line_id].line_id[0]) {
							...
							let  checkin_dt = new Date(self.reservation_lines1[res_line_id].checkin +' UTC');
							let checkout_dt = new Date(self.reservation_lines1[res_line_id].checkout +' UTC');

							let checkin_dtf = moment(checkin_dt).format('DD-MMM-YYYY HH:mm');
							let checkout_dtf = moment(checkout_dt).format('DD-MMM-YYYY HH:mm');
				...
				self.$el.find('#booking_calendar').fullCalendar({
					...
					select: function (start, end, jsEvent, view, resource) {
						...
						var form_context = session.user_context;
						form_context['checkin'] = start_date || null;
						form_context['checkout'] = end_date || null;
						form_context['hotel_resource'] = resource.id || null;
						form_context['shop'] = parseInt(hotel_id) || null;
						console.log("ggggggggggggggggggggggggggggggggg",form_context)
		...
		get_view_id: function () {
			console.log("get_view_id===============",this.view_ids)
