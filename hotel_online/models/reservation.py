...
class PartnerInherit(models.Model):
    ...
    def write(self, vals):
        if 'state_id' in vals and vals.get('state_id') in [False, '', None]:
            vals.pop('state_id')
...
class hotel_reservation(models.Model):
    ...
    payment_acquirer_id = fields.Many2one(
        'payment.acquirer', 'Payment Acquirer',  copy=False)
    payment_tx_id = fields.Many2one(
        'payment.transaction', 'Transaction',  copy=False)

    access_token = fields.Char(
        'Security Token', copy=False, default=_get_default_access_token)
